...
def predict_congestion(model, date_time):
    ...
    prediction_result, congestion_index, arrival_time = utils.predict_traffic()
    # Display the prediction result in route_text
    route_text.insert(tk.END, f"{prediction_result}\n")
    
    # Generate EDA plots
    # congestion_index_img, arrival_time_img = eda_utils.plot_eda(congestion_index, arrival_time)
    
    # # Display EDA plots in route_text
    # display_image(congestion_index_img)
    # display_image(arrival_time_img)
...
generate_button = ttk.Button(root, text="Predict Traffic", command=lambda: predict_congestion(selected_model.get(), datetime_entry.get()))
